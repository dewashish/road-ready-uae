...
export default function MockExamSelectionPage() {
  ...
  return (
    <div className="min-h-dvh bg-background pb-20 sm:pb-0">
      <Header showBack backHref={`/quiz/${vehicleType}`} title="Mock Exams" />
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex items-center gap-4 mb-2">
          ...
          <div className="flex-1">
            <h2 className="font-headline text-2xl font-bold text-primary">Mock Exams</h2>
            <p className="text-sm text-on-surface-variant">
              Simulate the real UAE driving test — {category?.name ?? 'Vehicle'}
            </p>
          </div>
        </div>
        ...
        {totalAttempts > 0 && (
          <div className="grid grid-cols-2 gap-3 mb-6 mt-4">
            <NeoCard level={1} shadow="none" className="text-center !p-3">
              ...
              <p className="font-label text-[10px] text-on-surface-variant uppercase tracking-wider">Best Score</p>
            </NeoCard>
            <NeoCard level={1} shadow="none" className="text-center !p-3">
              <p className="font-headline text-2xl font-bold text-tertiary">{totalAttempts}</p>
              <p className="font-label text-[10px] text-on-surface-variant uppercase tracking-wider">Total Attempts</p>
            </NeoCard>
          </div>
        )}
        ...
        <NeoCard level={1} shadow="none" className="!p-3 mb-6 mt-4">
          <div className="flex items-center gap-3">
            ...
            <p className="text-sm text-on-surface-variant">
              Each test has <span className="text-primary font-bold">45 questions</span> and a{' '}
              <span className="text-primary font-bold">30-minute</span> time limit. You need{' '}
              <span className="text-secondary font-bold">71%</span> to pass.
            </p>
          </div>
        </NeoCard>
        ...
        <div className="space-y-4">
          {MOCK_EXAMS.map((exam, idx) => {
            ...
            return (
              <Link key={exam.id} href={`/quiz/${vehicleType}/mock-exam/${exam.id}`}>
                <NeoCard
                  level={2}
