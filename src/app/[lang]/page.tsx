      ...
      <main id="main-content" className="max-w-7xl mx-auto px-4 sm:px-6">
        <div id="vehicle-section" className="relative z-30 mb-6 bg-background pt-6 -mt-[96vh]" ref={vehicleSectionRef}>
          ...
          <h2 className="font-headline text-lg sm:text-xl font-bold uppercase tracking-wider mb-6">
            <span style={{ WebkitTextStroke: '1px #f5ce53', color: 'transparent', paintOrder: 'stroke fill' }}>{dict.hero.uaes1}</span>{' '}
            <span className="text-primary">{dict.hero.theoryTest}</span>{' '}
            <span className="text-secondary">{dict.hero.platform}</span>
          </h2>
          ...
          >
            <h3 className="font-headline text-xl font-bold text-primary uppercase tracking-wider">{dict.home.chooseVehicle}</h3>
            ...
            <NeoCard level={1} shadow="secondary" className="mt-8">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <h4 className="font-headline text-base font-bold text-secondary">{dict.home.dailyChallenge}</h4>
                  <p className="text-sm text-on-surface-variant">{dict.home.dailyChallengeDesc.replace('{target}', String(dailyTarget))}</p>
                </div>
