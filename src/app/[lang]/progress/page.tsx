...
export default function ProgressPage() {
  ...
  const moduleInfo = MODULE_INFO.map(m => ({
    ...m,
    title: dict.modules[m.slug as keyof typeof dict.modules]?.title ?? m.title
  }))
      ...
      <main id="main-content" className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
          <NeoCard level={2} shadow="default" className="text-center !p-4">
            ...
            <p className="font-label text-[10px] text-on-surface-variant uppercase tracking-wider mt-1">{dict.progressPage.totalXp}</p>
          </NeoCard>
          <NeoCard level={2} shadow="default" className="text-center !p-4">
            <p className="font-headline text-3xl font-bold text-tertiary">{stats.avgPercent}%</p>
            <p className="font-label text-[10px] text-on-surface-variant uppercase tracking-wider mt-1">{dict.progressPage.avgScore}</p>
          </NeoCard>
          <NeoCard level={2} shadow="default" className="text-center !p-4">
            <p className="font-headline text-3xl font-bold text-success">{stats.totalCompleted}</p>
            <p className="font-label text-[10px] text-on-surface-variant uppercase tracking-wider mt-1">{dict.progressPage.quizzesDone}</p>
          </NeoCard>
          <NeoCard level={2} shadow="default" className="text-center !p-4">
            <p className="font-headline text-3xl font-bold text-secondary">{progress.currentStreak}</p>
            <p className="font-label text-[10px] text-on-surface-variant uppercase tracking-wider mt-1">{dict.progressPage.dayStreak}</p>
          </NeoCard>
        </div>

        <h3 className="font-headline text-lg font-bold text-primary uppercase tracking-wider mb-4">{dict.progressPage.moduleMastery}</h3>
        ...
        {stats.totalCompleted === 0 && (
          <NeoCard level={1} shadow="none" className="text-center !py-12 mt-6">
            ...
            <p className="mt-3 text-on-surface-variant">{dict.progressPage.noActivity}</p>
          </NeoCard>
        )}
      </main>
      <BottomNav />
