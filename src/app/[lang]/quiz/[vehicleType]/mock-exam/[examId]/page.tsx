...
export default function MockExamPage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      router.push(
        localePath(locale, `/quiz/${vehicleType}/mock-exam/${examId}/results?score=${state.score}&total=${state.questions.length}&time=${timeSpent}`)
      )
    }, delay)

    return () => clearTimeout(timer)
  }, [state.status, state.score, state.questions, state.sessionAnswers, state.timeRemaining, showTimeUp, vehicleType, examId, router, locale])
  ...
  return (
    <div className="min-h-dvh bg-background">
      <Header showBack backHref={localePath(locale, `/quiz/${vehicleType}/mock-exam`)} title={exam.title} />
